...
(function () {
	...
	function n(b, l, g, d, a, f, c) {
		function i(a) {
			...
			sigma.tools.drawRoundRect(b, Math.round(a.displayX - c / 2 - 2), Math.round(a.displayY - c / 2 - 2), Math.round(b.measureText(a.label)
				.width + 1.5 * a.displaySize + c / 2 + 4), Math.round(c + 4), Math.round(c / 2 + 2), "left");
			b.closePath();
			b.fill();
			b.shadowOffsetX = 0;
			b.shadowOffsetY = 0;
	...
	function u(b, l) {
		function g() {
			sigma.chronos.removeTask("node_" + c.id, 2)
		...
		this.draw = function (a, b, d, l) {
			...
			c.p.lastLabels = d;
			g();
			c.graph.rescale(c.width, c.height, 0 < a, 0 < b)
				.setBorders();
			...
			for (var f in c.domElements) "canvas" == c.domElements[f].nodeName.toLowerCase() && (void 0 == l[f] || 0 <= l[f]) && c.domElements[f].getContext("2d")
	...
	sigma.chronos = new function () {
		...
		function f() {
			...
			!m || !t.length ? a() : (x = (new Date)
				.getTime(), p++, D = y - r, u = r - D, h.dispatch("insertframe"), b(f))
		}
		...
		function c(a) {
			return a ? Object.keys(q)
				.filter(function (a) {
				return !!q[a].on
			})
				.length : Object.keys(q)
				.length
		}

		function j() {
			Object.keys(q)
				.length ? (h.dispatch("startgenerators"), h.unbind("killed", o), b(function () {
				for (var a in q) q[a].on = !0, i(q[a].task, a, !1)
			}), h.bind("killed", o)
				.runTasks()) : h.dispatch("stopgenerators");
			return h
		}
