 ...
 *
 */
searchPhonoListSelection = [];
(function ($) {
	$.addFlex = function (t, p) {
		var newWidth = window.innerWidth - 280;
		...
		p = $.extend({ //apply default properties
			...
			title: false,
			idProperty: 'id',
			pagestat: 'Visualizzazione da {from} a {to} su <b>{total} risultati totali</b>',
			pagetext: 'Pagina: ',
			outof: 'di',
			findtext: 'Trova: ',
			find_guidetext: '(premere il tasto INVIO per avviare la ricerca)',
			params: [], //allow optional parameters to be passed around
			procmsg: 'Attendere...',
			query: '',
			qtype: '',
			nomsg: 'Nessun elemento trovato',
			...
			searchBtn_title: "Cerca all'interno dei risultati",
			filterViewBtn_title: "Risultati per pagina",
			goToFirstBtn_title: "Vai alla prima pagina",
			goToPrevBtn_title: "Pagina precedente",
			goToNextBtn_title: "Pagina successiva",
			goToLastBtn_title: "Vai all'ultima pagina",
			reloadBtn_title: "Ricarica",
			
			selectOrUnselectAll_text: "Seleziona/deseleziona le righe mostrate"
		}, p);
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				if(data){
					var all_ids = data.total_ids;
					$("#all_ids").val(all_ids.toString());
				}
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						$(tbody).append(tr);
						$("#selected_row").val(searchPhonoListSelection);
						tr = null;
					});
			...
			buildpager: function () { //rebuild pager based on new properties
				$('.pcontrol input:first-child', this.pDiv).val(p.page);
			...
			selectAll: function() {
				filter_class = $("#all_ids").val();
				exploded = filter_class.split(",");
				var count_selected = 0;
				$(exploded, g.bDiv).each(function(item, value) {
					var id = value;
					
					if($("#row" + id).hasClass("trSelected")){
				...
				if(count_selected < p.rp){ // Select all
					$("tbody tr", g.bDiv).addClass("trSelected");
					$(".pUnselectAll", g.tDiv).removeClass("pSelectAll");
					$.each($(".function_btn", g.tDiv), function(){
						$(this).switchClass("disabled", "", 300);
					});
				} else { // Unselect all
					$("tbody tr", g.bDiv).removeClass("trSelected");
					$(".pUnselectAll", g.tDiv).addClass("pSelectAll");
					$.each($(".function_btn", g.tDiv), function(){
						$(this).switchClass("", "disabled", 300);
					});
					$("#selected_row").val("");
				...
				$('tbody tr', g.bDiv).each(function () {
					...
					$(this).click(function (e) {
						...
						if($(this).hasClass("trSelected")){
							$.each($(".disabled", g.tDiv), function(){
								$(this).switchClass("disabled", "", 300);
							});
							if (searchPhonoListSelection.indexOf(id) == -1) {
								searchPhonoListSelection.push(id);
								$("#selected_row").val(searchPhonoListSelection);
								if(searchPhonoListSelection.length == p.rp){
									$(".pUnselectAll", g.tDiv).removeClass("pSelectAll");
								}
							}
						} else {
							...
							if (searchPhonoListSelection.indexOf(id) != -1) {
								searchPhonoListSelection.splice(searchPhonoListSelection.indexOf(id), 1);
								$("#selected_row").val(searchPhonoListSelection);
								if(!$(".pUnselectAll", g.tDiv).hasClass("pSelectAll")){
									$(".pUnselectAll", g.tDiv).addClass("pSelectAll");
								}
							}
						}
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					if (btn.bclass && btn.bclass.indexOf("right") > 0){
						btnDiv.className = 'fbutton right';
					} else {
					...
					if (btn.btitle) {
						$('span', btnDiv).attr("title", btn.btitle);
					}
					if (btn.bid) {
					...
					if (btn.bclass && btn.bclass == "right"){
						$(tDiv2).append("<div class='btnseparator right'></div>");
					} else {
		...
		if (p.usepager) {
			...
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton" title="' + p.goToFirstBtn_title + '"><span></span></div><div class="pPrev pButton" title="' + p.goToPrevBtn_title + '"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.pagetext + ' <input type="text" size="4" value="1" /><input type="hidden" id="selected_row" value="" /><input type="hidden" id="all_ids" value="" /> ' + p.outof + ' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton" title="' + p.goToNextBtn_title + '"><span></span></div><div class="pLast pButton" title="' + p.goToLastBtn_title + '"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton" title="' + p.reloadBtn_title + '"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend('<div class="pGroup"><select name="rp" title="' + p.filterViewBtn_title + '">' + opt + '</select></div> <div class="btnseparator"></div>');
			...
			//add search button
			if (p.searchitems) {
				$('.pDiv2', g.pDiv).prepend('<div class="pGroup"> <div class="pSearch pButton" title="' + p.searchBtn_title + '"><span></span></div> </div>  <div class="btnseparator"></div>');
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext + 
						...
						"all'interno di: <select name='qtype'>" + sopt + "</select><i>" + p.find_guidetext + "</i></div>");
		...
		if ($('th', g.hDiv).length) {
			...
			if(p.hidable){
				$(g.nBtn).addClass('nBtn')
					.html('<div></div>')
					.attr('title', 'Nasconde/Visualizza Colonne')
					.click(function () {
						$(g.nDiv).toggle();
						return true;
					}
	...
	$.fn.flexSelectAll = function(p) { // function to select all
		return this.each( function() {
			if (this.grid) {
				this.grid.selectAll(); 
			}
  ...
  $.fn.flexSearchflexSearch = function(p) { // function to search grid
