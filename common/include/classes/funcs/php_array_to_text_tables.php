	...
	private function printRow($rowKey) {
		...
		for($line=1; $line <= $this->rs[$rowKey]; $line++) {
			...
			for($colKey=0; $colKey < count($this->keys); $colKey++) {
				print " ";
					$content = substr($this->rows[$rowKey][$this->keys[$colKey]], ($this->mW * ($line-1)), $this->mW);
					$dotted_content = (strlen($content) < strlen($this->rows[$rowKey][$this->keys[$colKey]]) ? substr($content, 0, -3) . "..." : $content);
				print str_pad($dotted_content, $this->cs[$colKey], ' ', (is_numeric($content) ? STR_PAD_LEFT : STR_PAD_RIGHT));
				print " " . $this->pcol;
			}
			print "\n";
		}
