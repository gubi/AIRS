<?php
/**
* Set a feed to active/unactive from scanning
* 
* PHP versions 5
*
...
if (isset($_GET["id"]) && trim($_GET["id"]) !== ""){
	if (isset($_GET["action"]) && trim($_GET["action"]) !== ""){
		...
		if (isset($_GET["type"]) && strlen(trim($_GET["type"])) > 0){
			$edit_feed_action =  $pdo->prepare("update `" . $primary_table . "` set `is_active` = ('" . addslashes($_GET["action"]) . "') where `id` = '" . addslashes($_GET["id"]) . "'");
