...
function create_link($text){
	...
	if (!strstr($text, "href")){
		if (strstr($text, "[http://")){
			$text = preg_replace("/\[(.*?) (.*?)\]/is", "<a href=\"$1\" title=\"$1\">$2</a>", $text); // all links
		} else if (strstr($text, "[[http://")){
			$text = preg_replace("/\[[(.*?) (.*?)\]]/is", "<a href=\"$1\" target=\"_blank\" title=\"$1\">$2</a>", $text); // all links
		} else {
			if (strstr($text, "http://")){
				*/
				// match protocol://address/path/
				$text = ereg_replace("[[:alpha:]]+(://)[^<>[:space:]]+[[:alnum:]/]", '<a href="\\0" target="_blank" title="\\0">\\0</a>', $text);
				// match www.something
				$text = ereg_replace("(^| )(www([.]?[a-zA-Z0-9_/-])*)", '\\1<a href="http://\\2" target="_blank" title="http://\\2">\\2</a>', $text);
				/*
			} else {
				print "ok";
				$text = preg_replace(array(
						...
						'`((?<!//)(www\.[\w+\-\.]+[[:alnum:]]/?))`si',
						'`(\w+\s\w+?)\s<?([a-z0-9_\-\.]+)@([a-z0-9_\-\.]+)>?`si'
					),
					array(
						'<a href="$1" target="_blank" title=\"$1\">$1</a> ',
						'<a href="http://$1" target="_blank" title=\"$1\">$1</a>',
						'<a class="mail" href="mailto:$1 &lt;$2@$3&gt;" title=\"$2@$3\">$1 &lt;$2@$3&gt;</a>'
					), $text); // all links
			}
		}
	}
