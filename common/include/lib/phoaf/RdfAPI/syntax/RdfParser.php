...
function & generateModel($base,$rdfBaseURI = false, $model = false) {
	...
	$input = @fopen($base,'r');
  if(!$input) throw new Exception("RDF Parser: Could not open File: $base. Stopped parsing.");
	...
	while(!$done)
	{
      ...
      throw new Exception("class: parser; method: generateModel): XML-parser-error $err_code in Line  $line of input document");
//	    trigger_error($errmsg, E_USER_ERROR);
	  }
	} 
	/* close file. */
