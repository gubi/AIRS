...
if (!isset($_GET["q"])){
	...
	$check_feed = $pdo->query("select * from `editorss_feeds` where `id` = '" . addslashes($GLOBALS["page_q"]) . "'");
	if ($check_feed->rowCount() > 0){
		...
		require_once("common/tpl/EditoRSS/feed_form.tpl");
	} else {
