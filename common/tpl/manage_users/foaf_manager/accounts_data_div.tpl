...
if(is_array($foaf_account) && count($foaf_account) > 0) {
	...
	foreach ($foaf_account as $accounts) {
		...
		$account_rows .= '<tr class="user_account"><td><table cellpadding="5" cellspacing="5" style="width: 100%;"><tr><td><input type="text" style="width: 25%;" name="user_account_type_' . $acc_c . '" rel="account" placeholder="Nome del servizio..." title="Nome del servizio (ad es. Facebook, Twitter, ecc...)" value="' . $accounts->get("foaf:name") . '" />&emsp;<input type="text" style="width: 25%;" name="user_account_uri_' . $acc_c . '" placeholder="Pagina del proprio account..." title="Indirizzo della pagina del proprio account" value="' . $accounts->get("foaf:accountServiceHomepage") . '" />&emsp;<input type="text" name="user_account_name_' . $acc_c . '" placeholder="Nome account..." title="Nome dell\'account" value="' . $accounts->get("foaf:accountName") . '" />&emsp;<a href="javascript: void(0);" class="btn add" style="float: none;" title="Aggiungi un altro account">+</a>' . $acc_remove_btn . '</td></tr></table></td></tr>';
	}
} else {
	$account_rows = '<tr class="user_account"><td><table cellpadding="5" cellspacing="5" style="width: 100%;"><tr><td><input type="text" style="width: 25%;" name="user_account_type_$account_start_no" class="account" placeholder="Nome del servizio..." title="Nome del servizio (ad es. Facebook, Twitter, ecc...)" value="" />&emsp;<input type="text" style="width: 25%;" name="user_account_uri_$account_start_no" class="account" placeholder="Pagina del proprio account..." title="Indirizzo della pagina del proprio account" value="" />&emsp;<input type="text" name="user_account_name_$account_start_no" class="account" placeholder="Nome account..." title="Nome dell\'account" value="" />&emsp;<a href="javascript: void(0);" class="btn add" style="float: none;" title="Aggiungi un altro account">+</a></td></tr></table></td></tr>';
}
